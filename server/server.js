...
io.on('connection', socket => {
  ...
  socket.on('createMessage', (newMsgData, callback) => {
    ...
    if (user && isRealString(newMsgData.text)) {
      io.to(user.room).emit(
        'newMessage',
        generateMessage(user.name, newMsgData.text)
      );
    }
    callback();
  });
  ...
  socket.on('join', (params, callback) => {
    ...
    socket.emit(
      'newMessage',
      generateMessage('Server', 'welcome to the chat!')
    );
    socket.broadcast
      .to(params.room)
      .emit('newMessage', generateMessage(`${params.name} has joined!`));
